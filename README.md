...
支持咪咕、网易云、酷我、QQ、酷狗、5sing、Jamendo 和 Spotify 共八个音乐源。**咪咕是唯一默认开启的来源**，其余在左侧来源列表一键启用（手机端位于顶部）。
...
前端回归测试不需要额外 Node.js 依赖，覆盖播放图标、面板状态、键盘滑块和搜索状态。真实音乐源仍需联网手动验证。
